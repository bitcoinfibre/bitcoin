...
static std::map<std::pair<uint64_t, CService>, std::shared_ptr<PartialBlockData> >::iterator RemovePartialBlock(std::map<std::pair<uint64_t, CService>, std::shared_ptr<PartialBlockData> >::iterator it) {
    ...
    for (const std::pair<const CService, std::pair<uint32_t, uint32_t> >& node : it->second->nodesWithChunksAvailableSet) {
            ...
            if (block.is_header_processing) {
                ...
                if (status != READ_STATUS_OK) {
                    ...
                    if (fBench) {
                        ...
                        for (const std::pair<const CService, std::pair<uint32_t, uint32_t> >& provider : chunksProvidedByNode) {
                    ...
                    if (!ProcessNewBlock(Params(), pdecoded_block, false, &fNewBlock)) {
                        ...
                        {
                            LOCK(cs_main);
                            have_prev = LookupBlockIndex(pdecoded_block->hashPrevBlock) != nullptr;
                        }
